...
class PedidosService {
  ...
  async createPedido(empleado_id, items) {
    ...
    try {
      ...
      for (const item of items) {
        const prod = await client.query('SELECT precio_venta FROM productos WHERE id = $1 AND activo = true', [item.producto_id]);
      ...
      const pedidoResult = await client.query(
        `INSERT INTO pedidos (empleado_id, estado, total, created_at, updated_at) VALUES ($1, 'pendiente', $2, NOW(), NOW()) RETURNING *`,
        [empleado_id, total]
      );
  ...
  async getPedidoById(id) {
    const r = await db.query(`SELECT p.*, u.nombre AS empleado_nombre FROM pedidos p LEFT JOIN usuarios u ON p.empleado_id = u.id WHERE p.id = $1`, [id]);
    if (!r.rows[0]) return null;
    const pedido = r.rows[0];
    const items = await db.query(`SELECT pi.*, pr.nombre AS producto_nombre FROM pedido_items pi JOIN productos pr ON pi.producto_id = pr.id WHERE pi.pedido_id = $1`, [id]);
  ...
  async updateEstado(id, estado) {
    return (await db.query(`UPDATE pedidos SET estado = $1, updated_at = NOW() WHERE id = $2 RETURNING *`, [estado, id])).rows[0];
  }
  ...
  async updatePedido(id, items) {
    ...
    try {
      ...
      for (const item of items) {
        const prod = await client.query('SELECT precio_venta FROM productos WHERE id = $1', [item.producto_id]);
        const precio = parseFloat(prod.rows[0].precio_venta);
        await client.query(`INSERT INTO pedido_items (pedido_id, producto_id, cantidad, precio_unitario, subtotal) VALUES ($1, $2, $3, $4, $5)`, [id, item.producto_id, item.cantidad, precio, precio * item.cantidad]);
      }
      await client.query('COMMIT');
  ...
  async descontarStock(pedidoId) {
    ...
    try {
      await client.query('BEGIN');
      const pedidoResult = await client.query(`UPDATE pedidos SET estado = 'completado', completado_at = NOW(), updated_at = NOW() WHERE id = $1 RETURNING *`, [pedidoId]);
      if (!pedidoResult.rows[0]) throw new Error('PEDIDO_NO_ENCONTRADO');
      const itemsResult = await client.query(`SELECT pi.producto_id, pi.cantidad FROM pedido_items pi WHERE pi.pedido_id = $1`, [pedidoId]);
      for (const item of itemsResult.rows) {
        const insumosResult = await client.query(`SELECT pi.insumo_id, pi.cantidad_requerida FROM producto_insumos pi WHERE pi.producto_id = $1`, [item.producto_id]);
        for (const pi of insumosResult.rows) {
          const cant = parseFloat(pi.cantidad_requerida) * item.cantidad;
          const stock = await client.query('SELECT stock_actual, nombre FROM insumos WHERE id = $1', [pi.insumo_id]);
          if (!stock.rows[0]) continue;
          if (parseFloat(stock.rows[0].stock_actual) < cant) throw new Error(`STOCK_INSUMO_INSUFICIENTE:${stock.rows[0].nombre}`);
          await client.query(`UPDATE insumos SET stock_actual = stock_actual - $1, updated_at = NOW() WHERE id = $2`, [cant, pi.insumo_id]);
          await client.query(`INSERT INTO stock_movimientos (insumo_id, pedido_id, tipo, cantidad, motivo, created_at) VALUES ($1, $2, 'consumo', $3, $4, NOW())`, [pi.insumo_id, pedidoId, cant, `Pedido completado [${pedidoId}]`]);
        }
      }
      await client.query('COMMIT');

...
// Devuelve { inicio, fin } para el período actual y { inicio, fin } para el anterior
...
async function calcularMetricas(inicio, fin) {
  // Ingresos: suma de totales de pedidos completados en el período
  ...
  // Costo de producción: sum(cantidad × costo_produccion) de los items de pedidos completados
  ...
  // Costo de inventario: entradas de stock registradas en el período
...
class ReportesService {
  // T-44 / T-45 / T-48: resumen financiero del período con variación vs período anterior
  async getResumen(periodo = 'mensual') {
    ...
    // T-48: variación porcentual de utilidad vs período anterior
    ...
    return {
      ...
      fecha_fin: fin,
      ...actual,
      // T-47: flag para el indicador visual del frontend
      utilidad_positiva: actual.utilidad > 0,
      periodo_anterior: {
        ...anterior,
        variacion_utilidad  // % de cambio respecto a este período
      }
    };
  }

  // T-49: métricas del turno del empleado (sin datos financieros — T-51)
  async getTurnoEmpleado(empleado_id) {
    const hoy = new Date(); hoy.setHours(0, 0, 0, 0);

    // Conteo de pedidos del turno actual del empleado
    ...
    // Top 3 productos más vendidos del día (global del negocio, no solo del empleado)
    ...
    return {
      ...
      // Sin ningún campo financiero (T-51)
    };
  }
}

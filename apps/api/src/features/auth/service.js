...
class AuthService {
  async login(email, password) {
    ...
    if (!await bcrypt.compare(password, usuario.password_hash)) throw new Error('CREDENTIALS_INVALID');
  ...
  async register({ nombre, email, password, tipo_registro = 'jefe', codigo_invitacion = null }) {
    // Hashear ANTES de cualquier query para no retener conexiones durante bcrypt

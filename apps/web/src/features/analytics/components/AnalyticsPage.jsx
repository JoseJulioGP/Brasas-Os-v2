...
export const AnalyticsPage = () => {
  ...
  const { productos, isLoading, error, fetchMargenes, clearError } = useAnalyticsStore();

  useEffect(() => { fetchMargenes(); }, []);

  const totalIngresos = productos.reduce((s, p) => s + (p.precio_venta || 0), 0);
  const totalCostos = productos.reduce((s, p) => s + (p.costo_produccion || 0), 0);
  const margenPromedio = totalIngresos > 0 ? ((totalIngresos - totalCostos) / totalIngresos) * 100 : 0;
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 animate-fade-in-up opacity-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-heading font-bold text-[#f5f0eb]">Análisis de Rentabilidad</h1>
        ...
        <ResumenCards totalProductos={productos.length} totalIngresos={totalIngresos} totalCostos={totalCostos} margenPromedio={margenPromedio} />
        ...
        {isLoading ? (
          ...
          <div className="grid lg:grid-cols-3 gap-6 animate-fade-in-up opacity-0 stagger-2">
              ...
              <div className="glass rounded-2xl overflow-hidden">
                ...
                {productos.length === 0 ? (
                  <div className="p-8 text-center">
                    ...
                    <button onClick={() => navigate("/menu")} className="mt-3 text-orange-400 text-sm font-medium font-body hover:underline">
                      Agregar productos
                    </button>
                  </div>
                ) : (
